...
## 2024-05-01 - Generator expressions vs List comprehensions for Thread evaluation
**Learning:** Eagerly evaluating `is_alive()` on a list of threads constructs a full list in memory and introduces significant overhead. E.g. `all([not(f.is_alive()) for f in threads])`.
**Action:** Prefer short-circuiting loops using generator expressions, e.g. `not any(f.is_alive() for f in threads)` to significantly reduce unnecessary thread state checks.
