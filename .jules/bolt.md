...
## 2024-05-24 - EAFP Pattern in Aggregators
**Learning:** In this codebase's statistical aggregators (e.g., CallStatistics), using the EAFP pattern (`try...except KeyError`) with local variable assignment for nested dictionaries in hot loops is significantly faster than using LBYL (`if key not in dict`) or `dict.setdefault`.
**Action:** Apply EAFP and local reference assignments instead of multiple dictionary lookups when aggregating statistics in hot loops.
