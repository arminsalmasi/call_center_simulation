...
## 2026-05-18 - Optimize statistical aggregators with EAFP
**Learning:** In this codebase's statistical aggregators (e.g., `CallStatistics`), using the LBYL pattern (`if key not in dict`) and repeated dictionary lookups adds execution overhead during hot loops (like processing thousands of events during simulation and benchmarking).
**Action:** Use the EAFP pattern (`try...except KeyError`) and assign nested dictionaries to local variables for dictionary operations in hot loops to significantly reduce execution overhead.
