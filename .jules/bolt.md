...
## 2024-05-14 - Double-Checked Locking in Hot Loops
**Learning:** In this codebase, frequent lock acquisitions on busy thread-safe models (e.g., checking `self._state` inside a lock in `Agent.try_assign`) create severe performance bottlenecks in hot loops.
**Action:** Apply a double-checked locking pattern (a lock-free fast-path state check before acquiring the lock) to significantly reduce thread contention overhead.
