...
## 2024-05-24 - Dataclass Serialization Bottleneck
**Learning:** In this codebase, using `dataclasses.asdict` for frequently serialized dataclass objects (e.g., `SimulationEvent` in `call_center/events.py`) causes severe performance overhead due to recursive deep-copying and reflection. Replacing it with manual dictionary creation (e.g., `{'kind': self.kind, ...}`) is approximately 15x faster and is the preferred optimization strategy.
**Action:** Avoid `dataclasses.asdict` for high-frequency serialization of simple dataclasses; use manual dictionary construction instead.
