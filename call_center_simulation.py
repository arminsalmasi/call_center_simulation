...
class CallCenterSimulation:
    ...
    def set(self, number_of_freshers, run_time, min_max_calls_per_wave, min_max_sleep_interval, min_max_call_duration):
        ...
        """
        if number_of_freshers < 0 or number_of_freshers > 1000:
    ...
    def run_simulation(self):
        ...
        try:
            ...
            while True:
                ...
                # OPTIMIZATION: Generator expression short-circuits thread state evaluations
                if not(technical_lead.is_alive()) and not(project_manager.is_alive()) and all(not(fresher.is_alive()) for fresher in freshers):
                    break
...
def main():
    ...
    try:
        ...
        # Validate inputs
        if args.number_of_freshers <= 0 or args.run_time <= 0 or args.min_calls_per_wave <= 0 or \
           args.max_calls_per_wave <= 0 or args.min_sleep_interval < 0 or args.max_sleep_interval < 0 or \
           args.min_call_duration <= 0 or args.max_call_duration <= 0:
            parser.error("All integer arguments must be strictly positive (> 0), except sleep intervals which can be 0.")
