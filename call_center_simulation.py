...
def main():
    ...
    try:
        ...
        if not (0 < args.number_of_freshers <= 1000):
            parser.error("number_of_freshers must be strictly positive and <= 1000")
        if not (0 < args.run_time <= 86400):
            parser.error("run_time must be strictly positive and <= 86400")
        ...
        if args.min_call_duration <= 0 or args.max_call_duration > 86400:
            parser.error("call_duration must be strictly positive and <= 86400")
