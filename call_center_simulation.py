...
class CallStatistics:
    ...
    def add_fresher_call(self, index, call_duration):
        ...
        """
        # Bolt: Optimize hot loop dict lookups using EAFP pattern
        try:
            entry = self.fresher_statistics[index]
            entry['counter'] += 1
            entry['call_duration'] += call_duration
        except KeyError:
...
def main():
    ...
    try:
        ...
        if args.max_calls_per_wave > 1000:
            parser.error("max_calls_per_wave must not exceed 1000")
