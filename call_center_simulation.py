...
class CallStatistics:
    ...
    def add_fresher_call(self, index, call_duration):
        ...
        """
        # BOLT OPTIMIZATION: Use EAFP pattern and local variable assignment to reduce dictionary lookups in hot loop.
        try:
...
class CallCenterSimulation:
    ...
    def set(self, number_of_freshers, run_time, min_max_calls_per_wave, min_max_sleep_interval, min_max_call_duration):
        ...
        if not (0 <= min_max_sleep_interval[0] <= min_max_sleep_interval[1]) or min_max_sleep_interval[1] == 0:
            raise ValueError("Invalid min_max_sleep_interval range: max_sleep_interval must be > 0 to prevent DoS")
...
def main():
    ...
    try:
        ...
        if args.max_sleep_interval <= 0:
            parser.error("max_sleep_interval must be strictly positive to prevent CPU spinning")
