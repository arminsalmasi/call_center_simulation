...
class CallStatistics:
    ...
    def add_fresher_call(self, index, call_duration):
        ...
        """
        # ⚡ Bolt Optimization: Use EAFP pattern (try/except) and reference local dictionary
        # to avoid multiple dictionary lookups in hot loops
        try:
...
class CallCenterSimulation:
    ...
    def set(self, number_of_freshers, run_time, min_max_calls_per_wave, min_max_sleep_interval, min_max_call_duration):
        ...
        if not (0 < number_of_freshers <= 1000):
            raise ValueError("number_of_freshers must be strictly positive and not exceed 1000")
    ...
    def _finish_remaining_calls(self, freshers, technical_lead, project_manager):
        ...
        while True:
            ...
            # Optimization: Using a generator expression with any() instead of all([]) short-circuits evaluation and avoids building a list of thread statuses in memory.
...
def main():
    ...
    try:
        ...
        # Security Enhancement: Validate upper bounds to prevent unhandled exceptions and DoS
        if args.number_of_freshers <= 0 or args.number_of_freshers > 1000:
            parser.error("number_of_freshers must be strictly positive and not exceed 1000")
        if args.run_time <= 0 or args.run_time > 86400:
            parser.error("run_time must be strictly positive and not exceed 86400")
