...
class CallStatistics:
    ...
    def add_fresher_call(self, index, call_duration):
        ...
        """
        # Optimization: Use EAFP pattern and local binding to minimize dictionary lookups in hot loop
        try:
...
class CallCenterSimulation:
    ...
    def set(self, number_of_freshers, run_time, min_max_calls_per_wave, min_max_sleep_interval, min_max_call_duration):
        ...
        if not (0 < run_time <= 86400):  # Max 1 day simulation
            raise ValueError("run_time must be strictly positive and up to 86400")
        if not (0 <= min_max_calls_per_wave[0] <= min_max_calls_per_wave[1] <= 10000):
            raise ValueError("Invalid min_max_calls_per_wave range")
        if not (0 <= min_max_sleep_interval[0] <= min_max_sleep_interval[1] <= 86400):
            raise ValueError("Invalid min_max_sleep_interval range")
        if not (0 < min_max_call_duration[0] <= min_max_call_duration[1] <= 86400):
    ...
    def _finish_remaining_calls(self, freshers, technical_lead, project_manager):
        ...
        while True:
            ...
            if not(technical_lead.is_alive()) and not(project_manager.is_alive()) and not any(f.is_alive() for f in freshers):
                break
...
def main():
    ...
    try:
        ...
        if not (0 < args.number_of_freshers <= 1000):
            parser.error("number_of_freshers must be strictly positive and up to 1000")
        if not (0 < args.run_time <= 86400):
            parser.error("run_time must be strictly positive and up to 86400")
        ...
        if args.min_call_duration <= 0 or args.max_call_duration > 86400:
            parser.error("call_duration must be strictly positive and up to 86400")
