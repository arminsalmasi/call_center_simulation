...
def main():
    ...
    try:
        ...
        if not (args.min_sleep_interval <= args.max_sleep_interval <= 86400):
            parser.error("max_sleep_interval must be greater than or equal to min_sleep_interval and <= 86400")
        if not (args.min_call_duration <= args.max_call_duration <= 86400):
            parser.error("max_call_duration must be greater than or equal to min_call_duration and <= 86400")
