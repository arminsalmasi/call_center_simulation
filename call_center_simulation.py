...
def find_free_fresher_index(bool_iterable):
    """Find an available fresher employee in the call center.

    Args:
        bool_iterable (iterable): Iterable of booleans indicating which freshers are available.

    Returns:
        int: Index of the first available fresher, -1 if no freshers are available.
    """
    for index, value in enumerate(bool_iterable):
        if value:
            return index
    return -1
...
class CallCenterSimulation:
    ...
    def set(self, number_of_freshers, run_time, min_max_calls_per_wave, min_max_sleep_interval, min_max_call_duration):
        ...
        # Enforce max sleep interval > 0 to prevent (0, 0) CPU spinning DoS loop
    ...
    def run_simulation(self):
        ...
        try:
            while True:
                ...
                for call in range(number_of_calls):
                    # Find indices of free freshers, -1 if none
                    # Optimize: Use generator expression for lazy evaluation of is_alive() to avoid unnecessary thread state checks
...
def main():
    ...
    try:
        ...
        if args.number_of_freshers < 0:
            parser.error("number_of_freshers must be non-negative")
        if args.run_time < 0:
            parser.error("run_time must be non-negative")
        ...
        if args.max_sleep_interval <= 0:
            parser.error("max_sleep_interval must be strictly positive")
