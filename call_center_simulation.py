...
class CallCenterSimulation:
    ...
    def _finish_remaining_calls(self, freshers, technical_lead, project_manager):
        ...
        while True:
            ...
            # Optimization: Using 'not any()' with a generator expression prevents unnecessary list allocation and fully utilizes short-circuiting for thread state checks.
...
def main():
    ...
    try:
        ...
        if args.number_of_freshers <= 0 or args.number_of_freshers > 1000:
            parser.error("number_of_freshers must be greater than 0 and up to 1000")
        if args.run_time <= 0 or args.run_time > 86400:
            parser.error("run_time must be greater than 0 and up to 86400")
