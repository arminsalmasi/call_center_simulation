...
def main():
    ...
    try:
        ...
        if args.number_of_freshers > 1000:
            parser.error("number_of_freshers must be at most 1000 to prevent resource exhaustion")
        if args.run_time <= 0:
            parser.error("run_time must be greater than 0")
        if args.run_time > 86400:
            parser.error("run_time must be at most 86400 to prevent excessive execution time")
